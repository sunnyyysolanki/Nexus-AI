package com.nexus.incident.filter;
...
public class GatewayTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (!StringUtils.hasText(expectedToken)) {
            log.warn("⚠️ [incident-service] nexus.internal-token not set — gateway auth disabled in dev mode");
            chain.doFilter(request, response);
            return;
        }
        ...
        if (!expectedToken.equals(incomingToken)) {
            log.warn("🚫 [incident-service] Blocked direct access: {} {} — invalid or missing X-Internal-Token",
